...
function hitung(e) {
    ...
    tBody.innerHTML += `<tr>
                ...
                <td>U1 = ${Z} / ${m.value} = ${(Z / parseInt(m.value)).toFixed(2)} = &radic;${(Z / parseInt(m.value)).toFixed(2)} = ${Math.sqrt(Z / parseInt(m.value)).toFixed(2)}</td>
            </tr>`
    for (let i = 1; i < bAcak.value; i++) {
        ...
        tBody.innerHTML += `<tr>
            ...
            <td>U${i + 1} = ${ZResult} / ${parseInt(m.value)} = ${(ZResult / parseInt(m.value)).toFixed(2)} = &radic;${(ZResult / parseInt(m.value)).toFixed(2)} = ${Math.sqrt(ZResult / parseInt(m.value)).toFixed(2)}</td>
        </tr>`
